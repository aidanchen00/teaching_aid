...
function VoiceControl({ onResults, onFilterChange }) {
  ...
  const getAgentState = () => {
    if (isSpeaking) return { state: 'speaking', label: 'Speaking', icon: '🔊' };
    if (isProcessing) return { state: 'thinking', label: 'Thinking', icon: '💭' };
    if (isListening) return { state: 'listening', label: 'Listening', icon: '🎤' };
    if (transcript || response) return { state: 'ready', label: 'Ready', icon: '✨' };
    return { state: 'idle', label: 'Idle', icon: '💤' };
  };
